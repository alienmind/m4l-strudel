...
var require_escodegen = __commonJS({
  "node_modules/.pnpm/escodegen@2.1.0/node_modules/escodegen/escodegen.js"(exports) {
    (function() {
      "use strict";
      var Syntax, Precedence, BinaryPrecedence, SourceNode, estraverse, esutils, base, indent, json, renumber, hexadecimal, quotes, escapeless, newline, space, parentheses, semicolons, safeConcatenation, directive, extra, parse4, sourceMap, sourceCode, preserveBlankLines, FORMAT_MINIFY, FORMAT_DEFAULTS;
...
Parser.extend = function extend() {
  ...
  while (len--) plugins2[len] = arguments[len];
  var cls = this;
...
// strudel/packages/transpiler/transpiler.mjs
...
function registerLanguage(type, config) {
...
var plugins = [];
function registerTranspilerPlugin(plugin) {
  plugins.push(plugin);
}
function getPlugins() {
...
function transpiler(input, options = {}) {
  ...
  const plugins2 = getPlugins().map((plugin) => plugin.walk?.(context));
  walk(ast, {
    enter(node, parent, prop, index) {
      if (blockBased && node.start !== void 0) {
  ...
  let output = import_escodegen.default.generate(ast);
  if (wrapAsync) {
    output = `(async ()=>{${output}})()`;
  }
...
var nonInlineWidgets = ["punchcard", "spiral", "scope", "pitchwheel", "spectrum", "pianoroll", "wordfall"];
function isVisualizerCall(node) {
  if (node.type === "CallExpression" && node.callee.type === "MemberExpression" && nonInlineWidgets.includes(node.callee.property?.name)) {
...
// strudel/packages/transpiler/plugin-kabelsalat.mjs
var import_escodegen2 = __toESM(require_escodegen(), 1);
function genExprSource(expr) {
  return import_escodegen2.default.generate(expr, { format: { semicolons: false } });
}
  ...
  walk: (context) => ({
    leave: function(node, parent, prop, index) {
      ...
      if (patternExprs.length) {
        ...
        let callee2 = node.callee;
        if (callee2.type === "ChainExpression") callee2 = callee2.expression;
        if (callee2.type === "MemberExpression") {
          return this.replace({
            type: "CallExpression",
            callee: workletMemberAst(callee2.object),
            arguments: workletArgs2,
            optional: false
          });
        }
        return this.replace({
...
// strudel/packages/transpiler/plugin-mini.mjs
var languageLiteral = {
  walk: (context) => ({
    enter: function(node, parent, prop, index) {
      if (!isLanguageLiteral(node)) return;
      context.miniLocations ?? (context.miniLocations = []);
      ...
      const offset2 = node.quasi.start + 1;
      if (emitMiniLocations) {
        const locs = language.getLocations(code, offset2);
        miniLocations.push(...locs);
      }
      this.skip();
      return this.replace(languageWithLocation(name, code, offset2));
    }
  })
};
var tidal = {
  walk: (context) => ({
    enter: function(node, parent, prop, index) {
      ...
      const offset2 = node.quasi.start + 1;
      if (emitMiniLocations) {
        const stringLocs = collectHaskellMiniLocations(raw, offset2);
        miniLocations.push(...stringLocs);
      }
      this.skip();
      return this.replace(tidalWithLocation(raw, offset2));
    }
  })
};
var backtick = {
...
function languageWithLocation(name, value, offset2) {
  return {
    type: "CallExpression",
    callee: {
      type: "Identifier",
    ...
    arguments: [
      { type: "Literal", value },
      { type: "Literal", value: offset2 }
    ],
    optional: false
  };
}
function collectHaskellMiniLocations(haskellCode, offset2) {
  return haskellCode.split("").reduce((acc, char, i) => {
    if (char !== '"') {
      return acc;
    }
...
function isMiniDisabled(offset2, miniDisableRanges) {
...
var collectMiniLocations = (value, node, miniLocations, input) => {
  const minilang = getLanguages().get("minilang");
  if (minilang) {
    const code = `[${value}]`;
...
// strudel/packages/transpiler/plugin-sample.mjs
...
// strudel/packages/transpiler/plugin-widgets.mjs
var widgetMethods = [];
function registerWidgetType(type) {
  widgetMethods.push(type);
}
var widgetTranspilerPlugin = {
  walk: (context) => ({
    enter: function(node, parent, prop, index) {
...
var sliderTranspilerPlugin = {
  walk: (context) => ({
    enter: function(node, parent, prop, index) {
      if (!isSliderFunction(node)) return;
      context.widgets ?? (context.widgets = []);
      context.sliders ?? (context.sliders = []);
...
var widgetTranspilerPlugins = [sliderTranspilerPlugin, widgetTranspilerPlugin];
...
function getWidgetID(widgetConfig) {
  let uniqueIdentifier;
  if (widgetConfig.from !== void 0 && widgetConfig.to !== void 0) {
...
// strudel/packages/transpiler/index.mjs
var evaluate2 = (code, transpilerOptions) => evaluate(code, transpiler, transpilerOptions);
export {
