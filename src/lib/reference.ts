...
export const CATEGORIES = ["Effects", "Modulation", "Mini-notation", "Patterns"] as const;
...
export function searchReference(entries: RefEntry[], q: string): RefEntry[] {
	...
	return entries.filter((e) => `${e.name} ${e.summary} ${e.example} ${e.note ?? ""}`.toLowerCase().includes(needle));
}
